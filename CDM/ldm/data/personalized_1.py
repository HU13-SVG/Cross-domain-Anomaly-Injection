...
class Personalized_mvtec_encoder(Dataset):
    def __init__(self,
        ...
        with open(os.path.join('coAno_anomaly_visa.txt'), 'r') as f:
        #with open(os.path.join('coAno-part.txt'), 'r') as f:
            data=f.read().split('\n')
            data.pop() # 这是添加的
            for i in data:
        ...
        self.num_images = len(self.data)  # len(self.data)=0?  435啊
    ...
    def __getitem__(self, idx):
        idx=idx%self.num_images  # 咋就成了0呢？
        example = {}
...
class Positive_sample_with_generated_mask(Dataset):
    ...
    def __init__(self,
        ...
        self.data_root = mvtec_path
        #self.mask_root='./generated_mask'  # 所有的生成都要重新训，generated_with_mask改为：1coAno_generated_mask
        self.mask_root = './1coAno_generated_mask_visa'
